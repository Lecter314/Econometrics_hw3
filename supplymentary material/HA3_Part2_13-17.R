...
df <- as.data.frame(read_excel("C:/Users/Bai Xinyi/Downloads/preprocessed_data_part2.xlsx"))
...
#14.Test the hypothesis that the effect of this variable is quadratic
logit4_quadra_capital = glm(formula = ranking ~ assets + capital + I(capital^2) + net_income + credit + 
      dif_assets + dif_capital + dif_income + dif_credit + I(dif_assets * 
                                                               assets) + I(dif_income * net_income), family = binomial(link = "logit"), 
    data = df)
summary(logit4_quadra_capital)  
#15.Test whether the effect of this explanatory variable is the same for 
#two groups: banks of Moscow and Moscow region and banks in the rest of Russia.
logit4_region_capital = glm(formula = ranking ~ assets + capital + I(capital*region) + net_income + credit + 
                              dif_assets + dif_capital + dif_income + dif_credit + I(dif_assets * 
                                                                                       assets) + I(dif_income * net_income), family = binomial(link = "logit"), 
                            data = df)
summary(logit4_region_capital)

#16.Compute the marginal effect at means. Compute the average marginal effect of capital on rating position.
install.packages('oglmx', dependencies = TRUE)
library('oglmx') 
our_data <- data.frame(ranking, region, assets, capital, net_income, credit, dif_capital,
                       dif_credit, dif_income, dif_assets)
results_log <- ologit.reg(ranking ~ assets + capital + net_income
                          + credit + dif_assets + dif_capital + dif_income + dif_credit + 
                            I(dif_assets*assets) + I(dif_income*net_income),                               # independent variables
                          data = our_data,                      # specify our data
                          savemodelframe = TRUE) 

margins.oglmx(results_log, ascontinuous = FALSE, atmeans = TRUE)
# AME - calculate AME
margins.oglmx(results_log, ascontinuous = TRUE, AME = TRUE)
